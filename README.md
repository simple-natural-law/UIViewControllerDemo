...
`UIViewController`类支持呈现一个视图控制器，并可用于所有视图控制器对象。我们可以从任何视图控制器呈现其他的任何视图控制器，但UIKit可能会将请求重新路由到不同的视图控制器。正在被呈现的视图控制器会在其和呈现它的原始视图控制器之间创建一种关系，其被称为原始视图控制器的`presentingViewController`，原始视图控制器被称为其`presentedViewController`。这种关系构成来 视图控制器层次结构的一部分，并保持原样直到视图控制器被从屏幕上撤除。
