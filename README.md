...
- 调用`viewForKey:`方法获取需要被添加或者移除的视图。视图控制器的视图可能不是在转场动画期间添加或者删除的唯一视图。presentation controller可能会将视图插入到其视图层次结构中，这些视图也必须被添加或者删除。`viewForKey:`方法返回包含需要添加或者删除的所有内容的根视图。
- 调用`finalFrameForViewController:`方法来获取被添加或者删除的视图的最终`frame`。

转场动画上下文对象使用`from`和`to`命名来标识转场中涉及的视图控制器、视图和frame。
