...
`UIViewController`类支持呈现一个视图控制器，并可用于所有视图控制器对象。我们可以从任何视图控制器呈现其他的任何视图控制器，但UIKit可能会将请求重新路由到不同的视图控制器。正在被呈现的视图控制器会在其和呈现它的原始视图控制器之间创建一种关系，正在被呈现的视图控制器是原始视图控制器的`presentingViewController`，原始视图控制器是正在被呈现的视图控制器的`presentedViewController`。这种关系构成了视图控制器层次结构的一部分，并保持原样直到视图控制器被从屏幕上撤除。
...
视图控制器的呈现风格控制其在屏幕上的外观。UIKit定义了许多标准的呈现风格，每种风格都有特定的外观和意图。我们也可以定义我们自己的自定义呈现风格。在设计应用程序时，请选择最适合我们正在尝试呈现的视图控制器界面的风格，并为视图控制器的`modalPresentationStyle`属性分配合适的值。
